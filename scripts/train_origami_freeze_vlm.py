...
class LoRALinear(nn.Module):
    """LoRA adapter around an existing Linear; the wrapped base stays frozen."""
...
def remap_base_linear_keys_for_lora(
    ...
    for key, value in state_dict.items():
        if key.endswith((".weight", ".bias")):
            ...
            if stem.endswith(target_suffixes):
                remapped[f"{stem}.base.{suffix}"] = value
                continue
        remapped[key] = value
    return remapped
    ...
    if accelerator.is_main_process:
        ...
        with open(os.path.join(save_dir, "training_args.json"), "w") as f:
            json.dump({
                ...
                "lerobot_root": getattr(args, "lerobot_root", ""),
                ...
                "action_lora": getattr(args, "action_lora", 0),
...
def train(args):
    ...
    ds_config["train_micro_batch_size_per_gpu"] = args.train_bsz_per_gpu
    ...
    if args.offload_optimizer_device:
        ds_config.setdefault("zero_optimization", {}).setdefault(
            "offload_optimizer", {})["device"] = args.offload_optimizer_device
    ...
    if getattr(args, "gradient_checkpointing", 0):
        model.model.gradient_checkpointing = True
        accelerator.print("Gradient checkpointing enabled for the MoT decoder.")
    ...
    if getattr(args, "action_lora", 0):
        n_lora_modules = inject_action_expert_lora(
            model,
            rank=args.action_lora_rank,
            alpha=args.action_lora_alpha,
            dropout=args.action_lora_dropout,
        )
        accelerator.print(
            f"Action expert LoRA enabled: {n_lora_modules} Linear modules, "
            f"rank={args.action_lora_rank}, alpha={args.action_lora_alpha}, "
            f"dropout={args.action_lora_dropout}"
        )
    ...
    for name, param in model.named_parameters():
        ...
        elif getattr(args, "freeze_vlm", 0):
            param.requires_grad = (
                name.startswith(trainable_when_vlm_frozen)
                or "lora_A" in name
                or "lora_B" in name
            )
            if ("_tactile" in name or "final_layer_tactile" in name) and freeze_tactile:
                param.requires_grad = False
    ...
    def _chunk_param_groups(named_params, weight_decay, max_numel=50_000_000):
        groups, params, n_params = [], [], 0
        for _name, param in named_params:
            p_numel = param.numel()
            if params and n_params + p_numel > max_numel:
                groups.append({"params": params, "weight_decay": weight_decay})
                params, n_params = [], 0
            params.append(param)
            n_params += p_numel
        if params:
    ...
    param_groups = (
        _chunk_param_groups(decay_params, args.weight_decay)
        + _chunk_param_groups(nodecay_params, 0.0)
    )
    try:
    ...
    if getattr(args, "data_format", "json") == "lerobot":
        from qwen_vla.origami_lerobot_dataset import OrigamiLeRobotDataset as TRexLeRobotDataset
    ...
    accelerator.print(
        ...
        f"{steps_per_epoch} dataloader steps/epoch, "
        f"{update_steps_per_epoch} optimizer steps/epoch, "
        f"effective_batch_size={effective_batch_size} "
        f"({args.train_bsz_per_gpu}/GPU x {dist.get_world_size()} GPUs "
        f"x grad_accum {accelerator.gradient_accumulation_steps})"
    )
    if getattr(args, "save_steps", 0) > 0:
        accelerator.print(f"Checkpointing every {args.save_steps} steps.")

    metric = TrainingMetrics(device=torch.cuda.current_device())
    global_step = int(getattr(args, "resume_global_step", 0))
    if global_step < 0:
        raise ValueError("--resume_global_step must be >= 0")
    if global_step > num_training_steps:
        raise ValueError(
            f"--resume_global_step ({global_step}) exceeds the configured "
            f"training schedule ({num_training_steps} steps)."
        )
    resume_epoch, resume_batches_in_epoch = divmod(
        global_step * accelerator.gradient_accumulation_steps,
        steps_per_epoch,
    )
    if resume_epoch >= args.n_epochs and global_step < num_training_steps:
        raise ValueError(
            f"Resume position resolves to epoch {resume_epoch}, but n_epochs="
            f"{args.n_epochs}."
        )
    if global_step:
        # Checkpoints contain model weights only, so restore the original LR
        # schedule position while intentionally starting a fresh optimizer.
        lr_scheduler.step(global_step)
        accelerator.print(
            "Weight-only resume: "
            f"global_step={global_step}, epoch={resume_epoch}, "
            f"skip_batches={resume_batches_in_epoch}, "
            f"lr={lr_scheduler.get_last_lr()[0]:.6g}. "
            "Optimizer momentum is not available in this checkpoint."
        )
    ...
    for epoch in range(resume_epoch, args.n_epochs):
        ...
        skip_batches = resume_batches_in_epoch if epoch == resume_epoch else 0
        epoch_dataloader = (
            accelerator.skip_first_batches(dataloader, skip_batches)
            if skip_batches else dataloader
        )
        it = (
            tqdm(
                epoch_dataloader,
            ...
            if accelerator.is_main_process else epoch_dataloader
        )
        ...
        for batch in it:
            ...
            if (getattr(args, "save_steps", 0) > 0
                ...
                save_checkpoint(model, processor, accelerator, args,
                                epoch, global_step,
                                getattr(dataset, "stats_data", None))
                last_save_step = global_step

            if getattr(args, "max_steps", 0) > 0 and global_step >= args.max_steps:
                accelerator.wait_for_everyone()
                save_checkpoint(model, processor, accelerator, args,
                                epoch, global_step,
                                getattr(dataset, "stats_data", None))
        ...
        if ((epoch + 1) % args.save_freq == 0 or epoch == args.n_epochs - 1) and last_save_step != global_step:
            accelerator.wait_for_everyone()
            save_checkpoint(model, processor, accelerator, args,
                            epoch, global_step,
                            getattr(dataset, "stats_data", None))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze_vlm", type=int, default=0,
                        help="1: freeze Qwen VLM backbone and train task heads/projections only.")
    ...
    parser.add_argument("--offload_optimizer_device", type=str, default="",
                        ...
                        help="Override DeepSpeed ZeRO optimizer offload device.")
    parser.add_argument("--action_lora", type=int, default=0,
                        help="1: freeze action expert base weights and train LoRA adapters on action Q/K/V/O and MLP Linear layers.")
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Global optimizer step represented by a weight-only resume "
            "checkpoint. Restores dataloader/LR progress; optimizer momentum "
            "cannot be restored because model checkpoints do not contain it."
        ),
    )
